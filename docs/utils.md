...
두 도구 모두 소스를 미리 훑어 **색인 파일**을 만들어 둡니다. 점프할 때 소스를 다시 읽는 것이 아니라 이 색인을 보므로, 색인이 없으면 아무것도 동작하지 않습니다.
...
# cscope — cscope.out 이 생긴다
find kernel user -name '*.c' -o -name '*.h' -o -name '*.S' > cscope.files
cscope -bq
```

`find` 로 시작하는 줄이 낯설 수 있는데, 색인에 넣을 파일 목록을 만드는 명령입니다.

| 조각 | 뜻 |
|---|---|
| `find kernel user` | `kernel/` 과 `user/` 두 디렉터리를 뒤진다 |
| `-name '*.c' -o -name '*.h' -o -name '*.S'` | 확장자가 `.c` **이거나**(`-o`) `.h` 이거나 `.S` 인 파일만 고른다 |
| `> cscope.files` | 고른 목록을 `cscope.files` 라는 파일로 저장한다 |

xv6 에서는 68개가 나옵니다 — `.c` 46개, `.h` 18개, `.S` 4개. 그다음 `cscope -bq` 가 이 목록을 읽어 색인을 만듭니다. `-b` 는 색인만 만들고 대화형 화면은 열지 않는다는 뜻, `-q` 는 조회를 빠르게 하는 보조 색인을 함께 만든다는 뜻입니다.

> ⚠️ **옵션 두 개가 중요합니다** `--exclude='*.asm'` 을 빼면 빌드 후 생기는 `kernel/kernel.asm` 이 색인에 섞여 `swtch` 같은 이름이 두 곳으로 잡힙니다. 그리고 cscope 를 그냥 `cscope -Rbq` 로 돌리면 `.c` 와 `.h` 만 색인하므로, 위처럼 파일 목록을 직접 만들지 않으면 `swtch.S` · `trampoline.S` · `entry.S` 를 찾지 못합니다.

> ⚠️ **색인은 그 시점의 사진입니다** `tags` 와 `cscope.out` 에는 색인을 만들던 때의 파일 이름과 줄 번호가 적혀 있습니다. 코드를 고쳐 줄이 밀리면 점프가 엉뚱한 줄로 가고, 새로 만든 함수는 색인에 없어 아예 찾지 못합니다. **실습으로 코드를 고친 뒤에는 위 두 명령을 다시 실행하세요.**
...
cscope — 아홉 가지를 찾을 수 있습니다. 커서를 이름 위에 두고 `:cs find <문자> <이름>` 을 칩니다.

| 문자 | 찾는 것 | 언제 쓰나 |
|---|---|---|
| `c` | 이 함수를 **부르는 곳** | 커널 읽기에 가장 유용. “누가 이걸 쓰나” |
| `g` | 정의된 곳 | `Ctrl-]` 와 같은 일 |
| `s` | 이 이름이 나오는 곳 전부 | 정의든 호출이든 가리지 않고 |
| `d` | 이 함수가 **부르는** 함수들 | 함수 하나를 처음 열었을 때 구조 파악 |
| `t` | 문자열 | panic 메시지에서 코드 역추적 |
| `e` | 정규식(egrep) 패턴 | `walk.*, *1` 처럼 패턴으로 |
| `f` | 파일 이름 | `:cs find f vm.c` 로 파일 열기 |
| `i` | 이 헤더를 `#include` 하는 파일 | 헤더 하나가 어디까지 퍼져 있나 |
| `a` | 이 변수에 **값이 대입되는 곳** | 값이 언제 바뀌는지 추적 |

실제로 자주 쓰는 것은 `c` · `g` · `s` 셋입니다. 나머지는 필요할 때 찾아 쓰면 됩니다.

```
:cs find t remap          " panic("mappages: remap") 을 찾는다
:cs find i defs.h         " defs.h 를 포함하는 파일 전부
:cs find a freelist       " kmem.freelist 에 값이 대입되는 곳
```

> ℹ️ **결과가 여럿이면** 아래에 목록이 뜨고 번호를 골라 들어갑니다. 목록을 다시 보려면 `:copen`, 다음 항목으로 넘어가려면 `:cn`, 이전으로는 `:cp` 입니다.

#### 실제로 이렇게 씁니다

**시나리오 1 — 처음 보는 함수의 쓰임새 파악**

「메모리 관리」 실습에서 `kalloc` 을 처음 열었다고 하겠습니다.

```
vim kernel/kalloc.c
:cs find c kalloc         " 누가 이걸 부르나 → 14곳이 나온다
```

**커널이 메모리를 얻는 통로가 열네 군데뿐**이라는 것이 눈으로 확인됩니다. 강의에서 그림으로 본 것과 같은 숫자입니다.

**시나리오 2 — panic 메시지에서 코드 찾기**

실습 중에 `panic: mappages: remap` 이 떴습니다.

```
vim kernel/vm.c
:cs find t remap          " 그 문자열이 있는 줄로 바로 이동
```

문자열은 심볼이 아니라 `Ctrl-]` 로는 찾을 수 없습니다. `t` 가 이때 쓰입니다.

**시나리오 3 — 함수 안을 따라 내려가기**

`walk()` 를 읽는 중에 `PTE2PA` 가 무엇인지 궁금합니다.

```
Ctrl-]                    " 매크로 정의로 (riscv.h)
Ctrl-t                    " 원래 자리로 돌아온다
Ctrl-w ]                  " 창을 나눠서 열면 양쪽을 함께 볼 수 있다
```

**시나리오 4 — 값이 언제 바뀌는지 추적**

`kmem.freelist` 가 어디서 바뀌는지 알고 싶습니다.

```
:cs find a freelist
  kernel/kalloc.c  kfree   61  kmem.freelist = r;
  kernel/kalloc.c  kalloc  76  kmem.freelist = r->next;
```

**딱 두 곳입니다** — 넣을 때와 뺄 때. 락이 지키는 것이 무엇인지가 여기서 분명해집니다.

**시나리오 5 — 헤더의 영향 범위 확인**

`defs.h` 를 고치면 무엇이 다시 컴파일될지 궁금합니다.

```
:cs find i defs.h         " defs.h 를 포함하는 파일 목록
```
